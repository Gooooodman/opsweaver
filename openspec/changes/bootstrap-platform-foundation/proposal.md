...
- 初始化单 Go Module Monorepo，提供 `aiops-server`、`worker`、`tool-gateway` 三个可独立启动的服务。
- 提供统一配置、结构化日志、健康检查、Prometheus 指标和英文错误响应。
- 提供 Docker Compose，在一个 PostgreSQL 容器中初始化 `aiops_db`、`tool_gateway_db`，并启动 Redis。
- 使用 GORM 访问数据库，使用显式版本化 SQL 管理两个数据库的迁移。
- 集成 Asynq：`aiops-server` 负责入队，`worker` 负责消费，Redis DB 0 用于队列。
