...
- [ ] 1.2 创建 `migrations/aiops/0003_mcp_servers.up.sql` 与 down migration，认证字段仅保存版本化密文
...
- [ ] 2.5 在 Worker 实现 `mcp.sync-tools` Handler，通过 Gateway 获取列表并回写 aiops-server 内部同步接口
...
- [ ] 3.2 创建 `migrations/aiops/0004_skills_workflows.up.sql` 与 down migration，保存不可变 Skill、Workflow、Prompt 内容
...
- [ ] 4.4 将 Runtime 装配到 Worker，并通过 aiops-server 内部 API 获取固定版本快照和回写步骤状态
